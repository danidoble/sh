...
EOF
    ...
    # PHP repository (Sury replaces the unsupported Ondřej PHP PPA on Ubuntu 26+)
    apt-get update || error_exit "Failed to update APT before adding the PHP repository"
    DEBIAN_FRONTEND=noninteractive apt-get install -y lsb-release ca-certificates curl \
        || error_exit "Failed to install PHP repository prerequisites"
    curl -fsSLo /tmp/debsuryorg-archive-keyring.deb \
        https://packages.sury.org/debsuryorg-archive-keyring.deb \
        || error_exit "Failed to download the Sury PHP repository keyring"
    dpkg -i /tmp/debsuryorg-archive-keyring.deb \
        || error_exit "Failed to install the Sury PHP repository keyring"
    ...
    apt-get update || error_exit "Failed to update APT after adding the Sury PHP repository"
...
install_packages() {
    ...
    local BASE_PACKAGES=(
        ...
        # CLI utilities
        btop fastfetch jq httpie tree ncdu tmux
...
alias ll="eza -la --icons --git --group"
